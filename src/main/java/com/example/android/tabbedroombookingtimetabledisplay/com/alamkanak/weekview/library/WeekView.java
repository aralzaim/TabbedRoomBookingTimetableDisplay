...
public class WeekView extends View {
    ...
    private void cacheEvent(WeekViewEvent event) {
        if (!isSameDay(event.getStartTime(), event.getEndTime())) {
            ...
            WeekViewEvent event1 = new WeekViewEvent(event.getId(), event.getName(), event.getStartTime(), endTime, event.getNamePurpose());
            event1.setColor(event.getColor());
            WeekViewEvent event2 = new WeekViewEvent(event.getId(), event.getName(), startTime, event.getEndTime(),event.getNamePurpose());
            ...
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    ...
                    try{
                        ...
                        return String.format("%s %02d/%d", dayName, date.get(Calendar.DAY_OF_MONTH), date.get(Calendar.MONTH) + 1);
                    }catch (Exception e){
                        e.printStackTrace();
                        return "";
                    }
